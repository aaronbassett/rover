...
async fn fetches_simple_page() {
    ...
    let page = fetch_url(&client, &url, SsrfLevel::Loopback, None, None)
        .await
        .expect("fetch ok");
...
async fn rejects_private_ip_at_ssrf_check() {
    ...
    let result = fetch_url(&client, &url, SsrfLevel::Strict, None, None).await;
...
async fn follows_redirects_and_records_final_url() {
    ...
    let page = fetch_url(&client, &start, SsrfLevel::Loopback, None, None)
        .await
        .expect("fetch ok");
