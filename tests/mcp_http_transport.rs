...
/// `tables.mode = "csv_file"` writes a CSV to the server's filesystem and
/// puts that absolute path in the returned Markdown — meaningless to a
/// caller in another container.
#[tokio::test]
async fn csv_file_tables_mode_is_refused_over_http() {
    let tmp = tempfile::tempdir().unwrap();
    let app = rover::mcp::http::router(common::http_state(tmp.path(), None).await);

    let body = Body::from(
        r#"{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"fetch_tool","arguments":{"url":"https://example.com/","tables":{"mode":"csv_file"}}}}"#,
    );
    ...
    assert!(
        ...
        "error must name the mode and the escape hatch, got: {text}"
    );
}

#[tokio::test]
async fn download_images_mode_is_refused_over_http() {
    let tmp = tempfile::tempdir().unwrap();
    let app = rover::mcp::http::router(common::http_state(tmp.path(), None).await);

    let body = Body::from(
        r#"{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"fetch_tool","arguments":{"url":"https://example.com/","images":{"mode":"download"}}}}"#,
    );
    let res = app.oneshot(mcp_request("POST", None, body)).await.unwrap();
