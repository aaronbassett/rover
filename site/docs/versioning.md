...
- **Minor** (`0.MINOR.z`) may break the MCP tool schema or the CLI flag set. Pin to an exact version and read the [CHANGELOG](https://github.com/aaronbassett/rover/blob/main/CHANGELOG.md) before bumping.
- **Patch** (`0.y.PATCH`) is bug-fix-only. It will not change the MCP tool schema or remove or rename CLI flags.
- **Pre-release** (`-rc.N`, `-beta.N`) ships to crates.io and GitHub Releases but does **not** update the Homebrew formula. Treat it as more volatile than a patch.
